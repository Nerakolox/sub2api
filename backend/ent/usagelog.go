...
type UsageLog struct {
	...
	// TpClientRef holds the value of the "tp_client_ref" field.
	TpClientRef *string `json:"tp_client_ref,omitempty"`
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldTpClientRef:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field tp_client_ref", values[i])
			} else if value.Valid {
				_m.TpClientRef = new(string)
				*_m.TpClientRef = value.String
			}
		case usagelog.FieldCreatedAt:
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	if v := _m.TpClientRef; v != nil {
		builder.WriteString("tp_client_ref=")
		builder.WriteString(*v)
	}
	builder.WriteString(", ")
